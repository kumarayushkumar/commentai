...
chrome.runtime.onInstalled.addListener(async (details) => {
  if (details.reason === 'install') {
    try {
      ...
      showBackgroundNotification(
        'Failed to initialize extension settings',
        'error'
      )
    }
    ...
    try {
      ...
      if (Object.keys(updates).length > 0) {
        await StorageService.setData(updates)
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  if (message.action === 'openSidePanel') {
    ...
    if (chrome.sidePanel) {
      ...
      // If openSettings is true, notify the side panel to open settings tab
      if (message.openSettings) {
        // Give the side panel time to load before sending the message
        setTimeout(() => {
